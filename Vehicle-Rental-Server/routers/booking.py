...
async def new_booking(booking: BookingCreate, currentUser: str = Depends(get_current_user)):
    ...
    conflict_maintenance = await maintenance_collection.find_one({
    ...
    if conflict_maintenance:
        notification.notify(
            title="Booking Alert",
            message=f'{booking.vehicle_name} is scheduled for maintenance during the date range. Try booking on different days!!',
            app_name='main',
            timeout=5
        )
        raise HTTPException(status_code = 400, detail= "Vehicle in maintenance for the selected date range")
    
    conflict_booking = await booking_collection.find_one({
        "vehicle_id": booking.vehicle_id,
        "$and": [
